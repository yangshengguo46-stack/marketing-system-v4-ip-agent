...
你只验证一件事：能否把一个真实业务对象作为语义起点，编译成一条具有完整人物因果的
一句话故事，同时正确决定该商业对象是否应当进入故事。
...
- `story_world`：具体人物之间发生的触发、欲望、行动、反馈、选择、代价与状态改变；
- `business_bridge`：观众看完故事后，可能把什么能力或意义归因给经营主体，以及它如何
  连接用户明确提供的商业目标。

商业对象进入故事前必须先做一次决定，`commercial_object_mode` 只能是：

- `absent`：对象只用来发现人的动作、关系和意义领地，随后从 `story_world` 和 `logline`
  彻底退出；不能换成另一件礼物、纪念物、信物或商品继续软植入；
- `causal`：对象的独特用途、规则或后果实际改变人物的行动、反馈、选择或状态；移除它或
  换成普通替代物会使具体因果边断裂。

不存在“出现一下但不影响剧情”的第三种模式。象征意义、材质联想和“更郑重”本身不能
证明对象是因果必需。必须在 `substitution_test` 中说明移除或替换对象后，哪条因果边会
改变；若没有任何因果边改变，就只能选择 `absent` 并把对象彻底删出故事。
...
6. 选择或遵守输入指定的 `commercial_object_mode`，完成替换测试；
7. 为故事人物建立触发、不同欲望、可判断目标、初始策略、现实反馈、策略变化、不能兼得
   的选择、代价和可见状态改变；
8. 把故事世界压成一条一句话故事；
9. 最后才单独说明业务连接。
