...
def test_sandbox_mkdir_precedes_uvicorn_launch(name):
    ...
    silently reintroduce the #3454 crash on a fresh checkout. The module-form
    uvicorn command matches the launch but not serve.sh's ``stop_all`` kill line.
    """
    lines = LAUNCHERS[name].read_text(encoding="utf-8").splitlines()
    launch_idx = next(
        (i for i, ln in enumerate(lines) if "uv run python -m uvicorn" in ln),
        None,
    )
    mkdir_idx = next((i for i, ln in enumerate(lines) if re.search(r"\bmkdir\b", ln) and "sandbox" in ln), None)

    assert launch_idx is not None, f"{name}: could not locate the module-form uvicorn launch line"
