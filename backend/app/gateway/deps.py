...
async def langgraph_runtime(app: FastAPI, startup_config: AppConfig) -> AsyncGenerator[None, None]:
    ...
    *startup-time* run-events configuration the underlying ``event_store``
    ...
    async with AsyncExitStack() as stack:
        ...
        app.state.minecontext_service = MineContextService(config=getattr(config, "minecontext", MineContextConfig()))
...
def get_personal_ip_data_lifecycle_service(
    request: Request,
) -> PersonalIPDataLifecycleService:
    val = getattr(request.app.state, "personal_ip_data_lifecycle_service", None)
    if val is None:
        raise HTTPException(
            status_code=503,
