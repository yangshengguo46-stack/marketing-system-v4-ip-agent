...
首次使用且只是在询问如何起步或定位时，IP Agent 的第一条可见回复会以普通
对话说明材料边界，并按人物、品牌、产品或组织邀请用户先讲整体“章节”；这
一步由服务端确定性完成，模型调用、联网找对标、Skill 加载和经营账本读取均
为零。后续只用最近的可见对话执行受限反映式访谈：先映照用户刚说的独有事实，
再决定只追问一个问题、停止，或把足够证据交给完整经营智能体。它不会默认追问
最早记忆或敏感经历，用户可以纠正、跳过、停止或限定为仅内部理解。
用户已经提供脚本、素材、链接或直接方案要求的具体任务会绕过或中断这段访谈。
