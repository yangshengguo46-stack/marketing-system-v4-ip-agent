...
1. `personal_ip_select_browser_account(account_id)` validates that the account
   belongs to the authenticated user and selects it as the concrete browser
   target for the current thread.
2. Selection does not narrow conversation authority. Portfolio questions still
...
3. The selection tool returns the platform's creator/start URL but never a local
   filesystem path.
4. DeerFlow Browser Control uses the account profile first. UI-TARS is the
   visual/desktop fallback when DOM browser actions cannot complete the task.
5. Publishing, sending, deleting and account-setting changes still require the
