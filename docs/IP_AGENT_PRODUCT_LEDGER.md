...
| M2 专业综合 | 已有真实证据和方法，与模型不能形成完整原创方案冲突 | 当前模型会错误改写证据实体，方法选择、综合与创作净增益尚未证明 | 当前主要矛盾 | 完整组达到 80 分且比 Chat 基线提高至少 15 分 |
...
| M1-E5 主账号代表作拆解 | 按本轮可见互动量选择前三条真实作品 `7658501922794432731`、`7551599574604811547`、`7657015996465719258`；均从精确作品详情取得媒体并完成 SHA-256、元数据、8 帧采样和联系表，时长分别为 70.867、22.533、58.967 秒；联系表人工检查与作品内容一致 | 主账号公网视频链通过；首次详情响应存在一次导航抖动，修复响应捕获竞态后重放成功，仍需在异类账号回放中验证稳定性 | 已解决 |
...
M1 的真实 Agent 回放已经证明模型能依次取得账号和三条视频证据，但最终回答仍把
“云沐荟足道官方号”错误改写为“云泉涧足道官方号”，并在缺少 ASR/OCR 时加入未经
完整语义证据支持的内容概括。当前占支配地位的问题不再是平台登录或工具可见性，
而是模型能否忠实读取证据、选择互不冲突的方法并形成专业原创结果。
