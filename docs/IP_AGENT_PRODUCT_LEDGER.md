...
| M2 专业综合 | 已有真实证据和方法，与模型不能形成完整原创方案冲突 | v2 已证明两项证据工具和方法发现入口在每次调用中真实可见，但豆包 2.0 只采集账号便越过视频深拆和方法读取，直接编造结论 | 当前主要矛盾 | 在不改题、不加提示词的前提下换第二模型做同题对照；只有取得视频证据并实际收到方法正文后，才评价专业综合能力 |
...
| M2-E2c 第二模型正确 treatment 对照 | 若跳过已可见工具是豆包 2.0 的模型适配问题，则只替换模型、保持冻结输入和 v2 能力面的较强模型应继续调用视频深拆 | 尚未执行；必须沿用 v2 的逐调用工具绑定、状态失败关闭和相同评分合同，不允许更改题目或增加提示 | 通过后再进入方法正文归因；若仍跳过视频证据，则判当前豆包模型族的动态工具链适配失败，转而比较其他模型或单一任务级 Evidence MCP，不堆提示词 | 验证中 |
| M2-E3 方法交付归因 | 用同一份封存 M1 证据、同一 Slash Skill 名称和同一单次综合调用，只替换 placebo 与组合方法正文，才能判断方法内容是否有效 | 尚未执行。两组均零工具、一次 LLM；组合方法只合并证据机制、IP 方向、受众反应、欲望行为、单集和场景对白，不注入无试拍的表演指导。Run Event 必须封存唯一 `skill_activation`、正文 SHA-256、实际能力面及 Token 差异；最终仍是一份连贯方案 | 只诊断“方法正文是否创造净增益”；即使通过也不能宣称完整取证→综合产品链通过，M3 不解冻 | 待转化 |
