...
| M2 专业综合 | 已有真实证据和方法，与模型不能形成完整原创方案冲突 | Evolving 已能完成账号采集、三视频深拆、看图和方法读取；专名改写已定位为 DeepSeek 适配器对 MCP 单文本块的二次 JSON 包装，修复后真实方舟精确复述 2/2 通过；当前未证明的是方法正文的净增益与综合效率 | 当前主要矛盾 | 用封存证据和单次综合调用隔离方法正文的净增益；不再用完整公网取证重跑掩盖方法归因 |
...
| Ark 专用模型适配器 | 当前 `PatchedChatDeepSeek` 只是复用最接近的 OpenAI-compatible 推理消息封装；本轮已在现有边界修复已证实的 Unicode/单文本块问题，现在重命名或换基类会同时扩大思考、工具、视觉和隐私回放变量 | Evolving 通过 M2 内容门槛后、正式提升前单独做 Ark 协议合同验收 |
