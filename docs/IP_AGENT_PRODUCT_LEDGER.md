...
| M1-E2 Evidence MCP | 只启动独立、最小的证据服务；公开路径失败后需用户在测试专用 Profile 手动登录 | 正式基线不启用；登录前只能验收诚实降级 | 验证中 |
...
审计过程只向抖音和开源仓库发起请求，未调用付费聚合 API，未提供或读取用户 Cookie。
隔离测试空间当前没有数据库或已授权抖音连接，所以登录态路径尚未验收。
