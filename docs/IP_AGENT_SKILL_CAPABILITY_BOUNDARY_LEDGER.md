...
| 复盘 | 方法修订 | 保留覆盖、比较结论和适用范围；样本量影响置信度但不形成服务器准入门 |

## 十五、运行时动态编排

编排单位不是 Skill，而是“用户当下要的决策或产物”。Lead Agent 每次只在内部
确认四件事：对象是人/品牌/产品/组织中的谁；要产出判断、脚本、成片还是
外部操作；当前材料属于事实、品牌/产品真相、社会情感洞察还是虚构剧情；
哪项新证据会改变决策。然后通过 DeerFlow 原生 `describe_skill -> read_file ->
tool result -> re-plan` 循环选择最小能力链，不再由中间件固定读取若干方法。

| 用户信号 | 起始能力 | 什么时候向下游交接 | 绝对不做什么 |
| --- | --- | --- | --- |
| 新用户只问“怎么开始” | 零工具叙事式邀请，之后每次一个高信息价值问题 | 已有可修正的主体真相、一个可观察证据和目标公众/行为后，进入候选方向 | 不先扫空账本、不强迫登录账号、不默认挖创伤 |
| 已连接自有账号并要“看看现在怎么样” | 当日采集、详细观测、账号诊断 | 样本、覆盖和时效足以分别内容问题、转化问题与平台资格问题后，交给策略或内容试验 | 不因低播放率直接判死号，不把缺失指标当零 |
| 只给对标名称 | 精确身份发现 | 身份未找到就请用户给主页链接并锁住下游；身份确认后再要 3 条代表作，拿到原作才交给视频提取 | 不把搜索摘要写成对标机制，不用通用 IP/剧情/传播方法填补证据空白，不换成泛行业攻略 |
| 给了对标视频、截图或录屏 | 客观时间轴描述、参考权利边界、视频模式提取 | 观察层与推断层分开，钩子/欲望/镜头/剪辑/声音/表演/转化机制有时间证据后，交给 IP 策略转译 | 不继续搜别人的二手概括，不复制受保护的表达表面 |
| 要账号方向 | 主体/业务真相、影响力目标、差异化和对标机制匹配 | 有“要改变谁的什么选择”、选择/相信理由、主动舍弃和可证伪试验后，交给长期系列或首条创作 | 不把“专家/避坑/日常”当差异化，不让平台代替内容论断 |
| 要故事、选题或脚本 | 欲望—目标—阻力—策略改变—有代价选择，再按需求进入主题/前提、情节信息、场景对白、类型和视听 | 稿件具有可观察行动和状态变化后，交给七层内容校准和表达方式路由 | 不强迫真实经营数据产生剧情，不拒绝明确标注的虚构/综合人物 |
| 用户上传自拍试片 | 镜头恐惧、声音、目光、身体、台词行动和可训练性评估 | 有可练问题和备选表达方式后，交给真人表演指导、混合或无脸/数字方案 | 不将紧张判成人格缺陷，不默认所有人都必须出镜 |
| 要直接做片 | 已批准脚本、表达模式、权利和预算合同，再进入分镜/素材/生成/声音/剪辑/QA | 完成精确哈希和交付 QA 后才交给发布 | 不让供应商回执反向定义创意，不绕过人工付费/发布授权 |
| 已发布或要复盘 | 预测版本、发布回执、同帖指标、评论/转化与对照 | 证据足够才交给规则更新或方法晋升 | 不用聊天记忆补回执，不把单条爆款写成永久规律 |

硬边界只保留在确定性问题：owner/account 授权、凭证、付费、发布、删除、权利、
合规、不可变回执、哈希和事实断言。创意质量使用方法、模型审查、对照试验和发布后
数据校准，不再使用全局关键词门禁把创意改写成服务器模板。
...
- 多组兄弟 Skill 已在本台账写明专业差异，但仍要通过真实回放证明模型能稳定触发和交接；
- 当前大部分 Skill 的保证级别仍是静态存在和局部测试，不是全栈行为回归。

当前代码已删除固定六方法旁路、私有审查室、广泛关键词门禁和确定性文案回退。
对标适配暂时不算完成产品路径；先定义模块的输入、输出、依赖和错误语义，再用
“贵厨笔记×黄金礼品”完整回放验收视频提取、欲望/剧情、内容校准、用户约束传递和
token/工具轨迹。未通过前不将“编排完成”写回产品总台账。
