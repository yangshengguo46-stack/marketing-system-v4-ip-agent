...
| AUD-BENCH-002 | P0 | Adapted-plan synthesis, orchestration and token control | A six-turn product/benchmark conversation originally made 23 model calls and consumed 618,963 tokens. The first remediation reduced the replay to 4,583 tokens, but it overcorrected: after one search it terminated the normal Agent loop, bypassed the first-party strategy/cinematic methods and used one flat sub-900-character form plus server-written generic conversion and pilot copy. The result was cheap but remained chat-level. | A plain greeting uses 0 model calls; benchmark evidence acquisition remains positively allowlisted and bounded. Product/entity + benchmark + adapted-plan history must then use the normal tool-result loop to discover and read a task-specific set of 3–6 first-party methods, generate 2–3 materially different directions, and pass them to a separate skeptical decision/continuity review. The customer result must be model-authored and product-specific: explicit evidence boundary, direction tradeoff, influence→behavior→economic hypothesis, proprietary truth, reason to believe, sacrifice, dramatic engine, repeatable series, conversion path, human/faceless modes and one shootable script with proof shots, qualitative signals and a falsification rule. Tests must prove actual method reads, bounded per-stage context, bounded retries, no fixed generic renderer and no fabricated representative-work judgment, customer story, metric, schedule, price/spec claim, private-channel assumption, viral guarantee or multiplier. Focused sync/async regressions plus a live gold-gift/贵厨笔记 replay record both quality evidence and total calls/tokens. | verifying |
