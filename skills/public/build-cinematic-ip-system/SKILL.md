...
> 研究隔离（2026-08-01）：本包仅保留供未来架构审查，未进入默认 IP Agent。
> 下文经营编排及工具名属于已退役实验，不是现役生产能力。当前合同见
> `IP_AGENT.md`。

用最小必要的子Skill完成当前目标。系统覆盖真实证据、欲望—行为0号协议、影视研究库、八轨96模块训练、创作生产和发布校准。不要每次都跑完整流水线，也不要在人物为何行动尚未清楚时直接堆结构、情绪、镜头和滤镜。

先调用 `personal_ip_startup_context`。若为 `new_owner`，直接承接用户当前一句话，
只收集会改变当前判断的下一项信息；不要读取完整经营驾驶舱，也不要检查空的策略、
发布、指标、复盘或视频台账。只有返回 `returning_owner`，或用户要恢复、发布、
查数据、复盘、继续视频时，才调用 `personal_ip_operating_cockpit` 读取全账号组合，
再为相关主体调用 `personal_ip_read_strategy_context`。项目原生主体策略、preflight、
发布回执、指标、复盘和视频制作台账是唯一运行状态；不得依据聊天重建
平行台账。账号仅在具体发布、采集或浏览器操作中作为目标。

## 先诊断阶段

| 当前材料 | 下一步 |
|---|---|
| 有人/品牌/产品/组织，但不知道应因何被记住和选择 | `$design-ip-differentiation` |
...
1. 建立人、品牌、产品或组织的主体范围，把真实证据、同意、限制和未知写入原生台账。
2. 建立当前可用的差异化判断：目标公众、希望形成的影响、替代选择、独有事实、选择理由、相信理由、主动舍弃、戏剧发动机和识别编码；缺项作为假设，不阻塞长期圣经或首条内容。
3. 从研究库选择3–6个互补机制；没有现成卡再深挖，不按名人拼贴。
4. 冻结生命周期欲望账本：主体、欲望、成功/失败状态、反欲望、进度证据和伦理边界。
5. 开发主题问题和强前提。
6. 产出继承同一差异化版本的IP圣经、人物关系与现实/伦理边界。
7. 为账号选一个主类型、最多两个辅助类型，并调用对应类型编剧室。
8. 定义账号情绪承诺、信息系统与第一季波形。
9. 每个样集先完成欲望—行为映射，再用三个不同事件做压力测试和场景/对白。
10. 设计调度与导演预演，按需调用摄影、光色、美术、声音与表演工种。
11. 剪辑后做连续性验收；发布前通过原生 preflight 冻结欲望映射和盲预测。
12. 登记发布、指标、影响力资产观察和复盘；据此修订差异化与局部方法，不等待固定观察门槛。

首批试播和发布前预测必须写明目标观众、证据等级、可观察机制假设、预期信号、
失败条件、平台分发假设、观察窗与不确定性。不得用多巴胺、镜像神经元、
蔡格尼克效应等名词替代证据，也不得把注意、情绪、扩散、转化与某个指标做一对一
因果绑定。“爆”是内容、观众、平台分配、竞争、时间与随机社会反馈共同产生的发布
结果，不是脚本自带属性。
...
- 发布前预测不可回写；
- 指标缺失保留为缺失；
- 多个独立观察和留出样本通常比一次爆款经验更可靠，但样本数不是继续工作的准入门。
